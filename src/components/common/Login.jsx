    ...
    try {
      if (isFirebaseConfigured && auth) {
        try {
          ...
          localStorage.setItem("token", user.accessToken || "demo_token")
          localStorage.setItem("role", userData?.role || role || "admin1")
          localStorage.setItem("userEmail", user.email || email)
        } catch (error) {
          console.warn("Firebase auth attempt bypassed for demo:", error)
        }
      }

      // Fallback demo session & instant navigation to dashboard
    ...
    try {
      if (isFirebaseConfigured && auth) {
        try {
          ...
          console.warn("Google Auth bypassed for demo:", error)
        }
      }

      // Direct navigation to dashboard for demo when clicking Continue with Google
      localStorage.setItem("token", "demo_token")
      localStorage.setItem("role", role || "admin1")
  ...
  return (
    <div className="text-on-surface min-h-screen flex antialiased relative w-full">
      <div className="fixed inset-0 z-0">
        <img alt="Relief Operations" className="w-full h-full object-cover" src={LoginscreenBG} />
      </div>
        ...
        <div className="glass-panel w-full max-w-xl rounded-2xl p-margin-mobile md:p-sm flex flex-col gap-md relative backdrop-blur-2xl bg-white/60">
          ...
          <div className="flex flex-col items-center text-center gap-xs">
            ...
            <h2 className="font-body-lg text-body-md md:text-body-lg max-w-md text-primary-container">
              Coordinating relief supplies to those who need them most.
            </h2>
            ...
            <div className="flex flex-wrap justify-center gap-sm mt-sm">
              <div className="inline-flex items-center gap-sm bg-white/60 backdrop-blur-sm px-sm py-xs rounded-lg border border-white/50">
                ...
                <span className="font-data-label text-data-label text-on-surface uppercase tracking-wider">Systems Operational</span>
              </div>
              <div className="inline-flex items-center gap-sm bg-white/60 backdrop-blur-sm px-sm py-xs rounded-lg border border-white/50">
                <div className="w-2 h-2 rounded-full bg-[#10b981] status-pulse" style={{ animationDelay: '0.5s' }}></div>
                <span className="font-data-label text-data-label text-on-surface uppercase tracking-wider">Matching Engine Online</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-sm">
            <div className="text-center space-y-base">
              ...
              <p className="font-body-md text-body-md text-primary-container">Sign in to the relief coordination platform.</p>
            </div>

            <form onSubmit={handleLogin} className="flex flex-col gap-sm">
              <div className="space-y-base">
                <label className="block font-data-label text-data-label text-primary-container" htmlFor="email">EMAIL ADDRESS</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-sm flex items-center pointer-events-none">
                    <span className="material-symbols-outlined text-on-surface-variant" style={{ fontSize: '20px' }}>mail</span>
                  </div>
                  <input className="input-field w-full pl-xl pr-sm py-sm border rounded-lg font-body-md text-body-md text-on-surface placeholder-on-surface-variant focus:ring-0 bg-white/90 border-outline-variant"
                    id="email" name="email" type="email" placeholder="patelsaumy@gmail.com" value={email} onChange={(e) => setEmail(e.target.value)} required />
                </div>
              </div>

              <div className="space-y-base">
                <label className="block font-data-label text-data-label text-primary-container" htmlFor="password">PASSWORD</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-sm flex items-center pointer-events-none">
                    <span className="material-symbols-outlined text-on-surface-variant" style={{ fontSize: '20px' }}>lock</span>
                  </div>
                  <input className="input-field w-full pl-xl pr-sm py-sm border rounded-lg font-body-md text-body-md text-on-surface placeholder-on-surface-variant focus:ring-0 bg-white/90 border-outline-variant"
                    id="password" name="password" type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required />
                </div>
              </div>

              <div className="space-y-base">
                <label className="block font-data-label text-data-label text-primary-container" htmlFor="role">SELECT ROLE</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-sm flex items-center pointer-events-none">
                    <span className="material-symbols-outlined text-on-surface-variant" style={{ fontSize: '20px' }}>badge</span>
                  </div>
                  <select className="input-field w-full pl-xl pr-sm py-sm border rounded-lg font-body-md text-body-md text-on-surface focus:ring-0 bg-white/90 border-outline-variant cursor-pointer"
                    id="role" name="role" value={role} onChange={(e) => setRole(e.target.value)}>
                    <option value="admin1">Relief Coordinator (Full Access)</option>
                    <option value="admin2">Supply Manager</option>
              ...
              <div className="flex items-center justify-between mt-xs">
                <div className="flex items-center">
                  <input className="h-4 w-4 rounded border-outline-variant text-primary-container focus:ring-primary-container bg-white/60" id="remember-me" name="remember-me" type="checkbox" />
                  <label className="ml-2 block font-body-sm text-body-sm text-primary-container select-none" htmlFor="remember-me">Remember me</label>
                </div>
                <div className="text-sm">
                  <a className="font-body-sm text-body-sm text-primary-container hover:text-secondary-container transition-colors font-medium" href="#">Forgot password?</a>
                </div>
              </div>

              <div className="flex flex-col gap-sm">
                <button className="w-full flex justify-center py-sm px-4 border border-transparent rounded-lg shadow-sm font-data-value text-data-value text-on-primary bg-primary-container hover:bg-primary focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-container transition-colors disabled:opacity-50"
